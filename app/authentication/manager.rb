  ...
  class Manager
    ...
    class << self
      ...
      def current_user
        ...
        # @current_user ||= User.find_by(id: decoded_auth_token.fetch(:user_id))
        User.find_by(id: decoded_auth_token.fetch(:user_id))
      end
      ...
      def decoded_auth_token
        # @decoded_auth_token ||= Authentication::TokenManager.decode(authorization_header)
        Authentication::TokenManager.decode(authorization_header)
      end
      ...
      def expiration_warning_response
        {
          body: {}, code: SUCCESS_CODE, headers: {
            'X-Expiration-Warning' => decoded_auth_token.fetch(:expiration_date).to_s
          }
        }
      end
